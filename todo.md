...
- **EMU SNR-candidates VizieR ID** *(parked, no good source found)*. `scripts/fetch_catalogs.py` tries six plausible VizieR IDs (`J/MNRAS/518/1273`, `J/MNRAS/535/4250`, `J/PASA/42/e005`, `J/MNRAS/500/2493`, `J/PASA/41/e003`, `J/MNRAS/524/L43`) and now logs which were tried when none match. Ball et al. EMU SNR papers don't appear to have VizieR mirrors yet. If you find one, add the ID to `emu_ids_tried` and the first match wins. Manual fallback: drop a JSON list at `data/catalogs.json::emu_candidates` per the warning hint.
