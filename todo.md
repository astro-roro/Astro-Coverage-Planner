...
- **Phase 5 — generic `docs/extensions.md` worked example.** The only remaining piece of the original plugin-architecture plan. ~30-60 lines of markdown showing how a third party would author an extension that registers a `CoverageSource`. Tiny — half a day at most.
- **`pix2world` TypeError on cursor hover (pre-existing).** Spammy `Uncaught TypeError: can't access property Symbol.iterator, i is undefined` from `aladin.js` traced through `hoverRaf` in `app.js`. Fires repeatedly during page init while the map isn't ready yet. Doesn't break anything visible but pollutes the console — likely needs an early-return guard in `hoverRaf` that skips the call until the map has finished its first render.
- **EMU SNR-candidates VizieR ID.** `scripts/fetch_catalogs.py` returns 0 rows for `emu_candidates`; the `cid` in the script is a best-effort guess from before there was time to confirm. Find the actual VizieR identifier for the EMU SNR-candidate catalogue (probably `J/...` — needs a literature check) and update.
- **ESO PNe (V/84) returns 0 entries in practice.** Same symptom as EMU above — the catalog appears in the rail with the right colour/shape but no markers render after `fetch_catalogs.py` runs. The pre-flight VizieR check during implementation showed V/84 returned 1143 rows, so the regression is in `fetch_eso_pne()` itself: probably a column-name mismatch when extracting `_RA.icrs` / `_DE.icrs` (column casing differs across VizieR responses). Re-run with logging on, inspect the actual columns the response carries, adjust the extractor.
