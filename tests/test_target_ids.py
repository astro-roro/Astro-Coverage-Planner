...
class TargetIdsTests(unittest.TestCase):

    def test_unchanged_library_keeps_every_id(self):
    ...
    def test_small_centre_drift_and_more_hours_keep_id(self):
    ...
    def test_new_target_gets_new_id_and_nobody_moves(self):
    ...
    def test_removed_target_retires_id_and_it_is_not_reused(self):
    ...
    def test_retired_id_comes_back_for_the_same_sky(self):
        without = [t for t in LIBRARY if t["objects"] != ["Sculptor"]]
    ...
    def test_split_keeps_id_on_part_with_most_integration(self):
    ...
    def test_merge_carries_override_across(self):
    ...
    def test_merge_onto_target_without_override_moves_it(self):
        tmp_path = Path(tempfile.mkdtemp())
    ...
    def test_carry_over_skips_missing_file(self):
        tmp_path = Path(tempfile.mkdtemp())
    ...
    def test_target_near_ra_zero_matches_itself(self):
    ...
    def test_target_near_pole_matches_itself(self):
    ...
    def test_missing_previous_manifest_falls_back_to_ledger(self):
    ...
    def test_first_run_adopts_live_ids_as_they_stand(self):
    ...
    def test_no_previous_anything_numbers_from_one(self):
        new, led, rep = rescan(None, LIBRARY)
        assert [t["target_id"] for t in new] == [1, 2, 3, 4, 5]
        assert rep["previous_source"] == "none"


    def test_small_target_inside_old_wide_field_is_not_the_same_target(self):
    ...
    def test_ledger_round_trip_is_atomic(self):
        tmp_path = Path(tempfile.mkdtemp())
        _, led, _ = rescan(LIBRARY, LIBRARY)
    ...
    def test_merge_chain_resolves_to_latest_survivor(self):
        led = ti.empty_ledger()
        led["merges"] = {"3": 5}
        led["next_id"] = 10
    ...
    def test_large_shuffled_library_keeps_every_id(self):
        import random
        rng = random.Random(7)
        lib = []
        while len(lib) < 300:
    ...
    def test_describe_report_counts(self):
