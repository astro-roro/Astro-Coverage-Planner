...
def test_catalog_names_extracted(stem, expected):
    assert object_from_filename(stem) == expected
...
def test_non_targets_return_none(stem):
    assert object_from_filename(stem) is None


def test_messier_not_matched_in_camera_model():
    """ASI1600MM contains M1600 — must not produce a false Messier match."""
    assert object_from_filename("Light_ASI1600MM_300s") is None
...
def _make_fits_with_downsampled_wcs(
    ...
    *,
    ...
    # WCS in the downsampled pixel grid
    hdr["CTYPE1"] = "RA---TAN"
    hdr["CTYPE2"] = "DEC--TAN"
...
def test_downsampled_wcs_uses_imagew_imageh():
    ...
    assert meta["ok"]
    assert meta["has_wcs"]
    # Should be very close to the target — not 5+ degrees off
    ...
    assert sep.deg < 0.1, f"Separation {sep.deg:.2f}° — should be < 0.1°"


def test_full_resolution_wcs_unaffected():
    ...
    assert result.separation(target).deg < 0.1


def test_no_imagew_falls_back_to_naxis():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        pix_scale = 206.265 * 3.8 / 334 / 3600  # degrees per pixel
...
def test_object_fallback_from_filename():
    ...
    assert meta["ok"]
    assert meta["object"] == "NGC 6960"


def test_object_from_header_takes_precedence():
    ...
    assert meta["object"] == "Western Veil"
